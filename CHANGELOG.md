...
- **ZImageUpscalePlus v2**: sigma preset 选择（`alpha_3..alpha_10` + `bravo_8`） + tiled `upscale_model` 接通（需 `vae` 输入，VAE round-trip + tiled scale OOM retry） + partial denoise 末级精修（`force_full_denoise=False`） + Karras-EDM `noise_inverse` stage 间桥接。共享新 helper 模块 `_sigma_presets` / `_sample_common` / `_noise_inverse`。`ZImageTurboProgressive._stage_denoise` 改为调 `_sample_common._stage_sample_with_sigmas`（行为 1:1 等价保留）。Inputs 新增 `vae?` / `sigma_preset`（default `alpha_8`），末级精修走用户 `denoise`（default 0.6 partial denoise）。

- **ZImageUpscalePlus v3**: 加 text conditioning hook（`positive?` / `negative?` 可选 CONDITIONING inputs，默认 `cfg=1.0` 下 guidance 被屏蔽但保留扩展位）+ `sampler_name` 透传到 `_stage_sample_with_sigmas`（helper 新增 `sampler_name: str = ""` kwarg，为 `DISCARD_PENULTIMATE_SIGMA_SAMPLERS` 优化预留 hook）。`ZImageTurboProgressive._stage_denoise` 同步加 `sampler_name` 参数透传。所有 caller 默认 `sampler_name=""`，v3 不引入任何行为变更。

- **`ZImageUpscalePlus` node** (menu `ZSimple-Nodes/image`) — progressive upscale + final-stage refinement for existing latent using Z-Image Turbo. Shares `_partition_common` (locked noise handoff) with `ZImageTurboProgressive`; bilinear upscale between stages, optional `upscale_model` (v2 TODO), `refinement_model` for final pass. v1 supports bilinear + Z-Image refinement; v2 TODO: tiled `upscale_model` integration + sigma schedule slicing. Inputs: `latent_input` / `model` / `upscale_factor` / `max_step_scale` / `tail_steps_first_upscale` / `tail_steps_last_upscale` / `upscale_model?` / `refinement_model?` / `sampler?` / `scheduler?` / `denoise?` / `seed`. Output: `latent`.

- **Refactor**: extracted `_PARTITION_CACHE` + 6 locked-noise / partition helper functions (`_build_partition_map` / `_reduce_height` / `_expand_height` / `_reduce_width` / `_expand_width` / `_project_to_coarse_subspace` / `_lift_noise` / `_locked_noise_from_prev`) into shared `nodes/_partition_common.py`. `ZImageTurboProgressive` now imports from `_partition_common`; byte-level behavioral equivalence preserved (cache key `(low_n, high_n, device.type)` unchanged). `_PARTITION_CACHE` thread-safety caveat from CLAUDE.md applies (now shared between `ZImageTurboProgressive` and `ZImageUpscalePlus`).
