...
- **PromptEnhancePlus image handling**:
  ...
  - **Phrase output style** (`target_model="Phrase"`): outputs a single-line Chinese comma-separated phrase prompt for SD / FLUX / Z-Image etc. Independent of the model-specific caption templates — backed by `model_system_prompt/phrase_t2i.md`. T2I-only; `image` / `video` / `audio` ports are ignored. `_strip_think_blocks` keeps the output clean of chat preamble.
  - **PhraseEN output style** (`target_model="PhraseEN"`): outputs a single-line ENGLISH comma-separated phrase prompt (no full sentences, no subject-verb structures) for SD / FLUX / Z-Image etc. Backed by `model_system_prompt/phrase_en.md`. T2I-only; `image` / `video` / `audio` ports are ignored. Format requirements are baked into the template as a numbered list (user-provided content).
  - **Flux output style** (`target_model="Flux"`): Flux-tuned variant — same ENGLISH comma-separated phrase format as PhraseEN, with explicit guidance to add cinematic details (camera view, spatial layout, render quality terms). Backed by `model_system_prompt/phrase_flux.md` (user-provided content). T2I-only; `image` / `video` / `audio` ports are ignored.
